...
{
    internal sealed class MemberDetailRepository : IMemberDetailRepository
    {
        private readonly IDbConnectionFactory _dbConnectionManager;

        public MemberDetailRepository(IDbConnectionFactory dbConnectionManager)
        {
        ...
        public async Task<MemberDetail?> GetByIdAsync(string id)
        {
            ...
            var cmdDef = new CommandDefinition(commandText, parameters);
            using var conn = await _dbConnectionManager.OpenAsync().ConfigureAwait(false);
            return await conn.QueryFirstOrDefaultAsync<MemberDetail>(cmdDef).ConfigureAwait(false);
        }
